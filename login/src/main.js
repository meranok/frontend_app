...
story: "Harari takes us on an epic journey through the history of our species, from our emergence as insignificant apes to our rise as masters of the planet. He examines the cognitive, agricultural, and scientific revolutions that have shaped human existence, challenging our fundamental assumptions about what it means to be human."
        ...
        function showCartModal() {
            ...
            cart.forEach(item => {
                ...
                totalCartPrice += parseFloat(itemTotal);
                cartItemsHtml += `
                    <div class="cart-item">
            ...
            <div class="modal-content purchase-form-modal">
                <h3 class="modal-title">Your Card</h3>
                <div id="cart-items-container">${cartItemsHtml}</div>
                <p class="total-price">Total Card Value: $${totalCartPrice.toFixed(2)}</p>
                <div class="purchase-buttons">
                    <button id="go-back-from-cart" class="go-back-button">Go Back</button>
                    <button id="checkout-button" class="buy-button">Checkout</button>
                </div>
            </div>
        `;
        messageBox.classList.add('visible');
...
function addToCart(book) {
    ...
    showTempMessage(`Added "${book.title}" to card!`);
}
    ...
    messageBox.innerHTML = `
        <div class="book-details-modal">
            ...
            <div class="modal-buttons">
                <button id="add-to-cart-button" class="buy-button">Add to Card</button>
...
messageBox.innerHTML = `
    ...
    `;
        ...
        function renderBooks(booksToRender) {
            booksGrid.innerHTML = '';
            booksToRender.forEach(book => createBookCard(book));
        }
        ...
        document.addEventListener('click', (event) => {
            ...
            if (event.target.id === 'go-back-from-cart') {
                hideMessageBox();
            }
            if (event.target.id === 'checkout-button') {
                showPurchaseOptions({ title: "Your Card" }); 
            }
        });
